...
Метод для обновления машины, привязанной к определенной RFID метке. Для использования надо указать существующую 
рфид метку в поле rfid_value. По этому идентификатору будет найдена машина (если она привязана к метке), и поля этой
машины будут изменены на поля, переданные в запросе. Если передать в запросе гос номер, который уже есть в базе, то
будет ошибка.
...
фоновая задача, которая самостоятельно обновить статус в ACTIVE как только текущая дата станет больше чем дата из
запроса
...
### Снятие rfid метки с паузы - `PUT /ui/rfid/resume/{rfidId}`

Метод который снимает с паузы рфид, переводя его в статус ACTIVE

**Список параметров**
`@PathVariable Long rfidId` - передается в урл, номер рфид метки

**Формат ответа**
HttpStatus.OK

### Получение rfid с привязанной машиной - `GET /ui/rfid/allRfidsWithCars`

получение rfid вместе с привязанной машиной
...
"content": [
  ...
  а также набор полей для пейджинга
  ```
...
данный метод нужен для принудительного открытия шлагбаума. на фронте скорей всего будет схемка, с расположением
шлагбаумов(нужно продумать этот момент), охранник выбирает нужный шлагбаум, далее открывается форма, где он заполняет
номер проезжающего автомибиля и тип события(вьезд или выезд).
